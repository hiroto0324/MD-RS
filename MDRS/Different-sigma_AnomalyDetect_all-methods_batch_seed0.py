                ...
                if n == reservoir.train_length - 1:
                    if online == True:
                        ...
                        if precision_mode == 'zero':
                            ...
                            # cov_mat = np.cov(x_mat, rowvar=False) # 平均で補正あり, 正則化なし
                            # cov_mat = reservoir.train_length * np.cov(x_mat, rowvar=False) + reservoir.alpha * np.identity(reservoir.N_P) # 平均で補正あり, 正則化あり
                            cov_mat = (x_mat - reservoir.mu).T @ (x_mat - reservoir.mu) + reservoir.alpha * np.identity(reservoir.N_P) # 平均で補正あり, 正則化あり
...
if __name__ == "__main__":
    ...
    # ratio = 0.3 # 名残りあり
    # ratio = 0.7 # 割と良い
    # param['tau'] = np.concatenate((np.full(int(param['Nx']*ratio), 0.01), np.full(int(param['Nx']*(1-ratio)), 0.0225))) # 二つの値を混在させる→一番いい？
    ...
    # tau_list = np.linspace(0.01, 0.0225, 11)
    # tau_list = np.linspace(0.01, 0.10, 10)
    # tau_list = np.linspace(0.01, 0.035, 11)
    # print('tau list:', tau_list)
    sigma_list = np.linspace(0, 0.1, 11)
    sigma_list = np.round(sigma_list, 2)

    # for iteration_idx in range(6):
    global seed
    # for iteration_idx, tau in enumerate(tau_list):
    for iteration_idx, sigma in enumerate(sigma_list):
        ...
        '''
        folder_path = f'../UCR_Anomaly_FullData/data/noised/sigma{sigma:.2f}'
        result_path = f'../UCR_Anomaly_FullData/results/Reservoir/noised/sigma{sigma:.2f}'
        plot_path = f'../UCR_Anomaly_FullData/plots/Reservoir/noised/sigma{sigma:.2f}'
        ...
        for filename in os.listdir(folder_path):
            ...
            if match:
                ...
                if method == 'SPE':
                    ...
                    # anomaly_score = AnomalyDetect_MDRS(normalized_data, reservoir, precision_mode='zero', continual_learning=continual_learning, online=online)
                    anomaly_score = AnomalyDetect_MDRS(normalized_data, reservoir, precision_mode='mean',
                                                       continual_learning=continual_learning, online=online)
                ...
                np.savetxt(os.path.join(result_path, filename.replace(".txt", f"_{method}.txt")), anomaly_score)
                ...
                plt.savefig(os.path.join(plot_path, filename.replace(".txt", f"_{method}_noised_sigma{sigma:.2f}.png")))
                plt.close()

        time_path = f'../UCR_Anomaly_FullData/analysis/Reservoir/runtime_{method}_noised_sigma{sigma:.2f}.csv'
