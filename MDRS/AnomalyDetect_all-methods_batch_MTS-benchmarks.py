...
class Reservoir:
    ...
    def run_one_step(self, n, input, mode, online=False):
        ...
        if reservoir.Ni == 1:
            ...
            sum += self.Wi @ input
            # has_nan = np.isnan(input).any()
            # print(f"{n} input contains NaN:", has_nan)
            # print(input)
...
if __name__ == "__main__":
    ...
    # plot_path = f'../{benchmark}/plots/Reservoir'
    folder_path = f'../benchmarks/{benchmark}/data'
    result_path = f'../benchmarks/{benchmark}/results/Reservoir'
    plot_path = f'../benchmarks/{benchmark}/plots/Reservoir'
    ...
    # for filename in os.listdir(filename_list_path):
    for filename in filename_list:
        ...
        # elif benchmark in ['SMD', 'SMAP', 'MSL', 'PSM']:
        else:
            if benchmark in ['SMD']:
            ...
            # elif benchmark in ['WADI']:
            #     if filename == 'WADI.A1_9 Oct 2017':
            #         df_train = pd.read_csv(f'{folder_path}/{filename}/WADI_14days.csv')
            #         df_test = pd.read_csv(f'{folder_path}/{filename}/WADI_attackdata.csv')
            #
            #     elif filename == 'WADI.A2_19 Nov 2019':
            #     df_train = pd.read_csv(f'{folder_path}/SWaT_Dataset_Normal_v0.csv')
            #     data_train = df_train.iloc[:, 1:-1].to_numpy() # 1列目と最後の列を除外
            #     df_test = pd.read_csv(f'{folder_path}/SWaT_Dataset_Attack_v0.csv')
            #     anomaly_label = df_test.iloc[:, -1].to_numpy().flatten()
            ...
            if benchmark in ['PSM']:
                ...
                # 改めてNaNが含まれているかチェック
                # contains_nan_after = np.isnan(data_array).any()
                # print(contains_nan_after)
        ...
        # np.savetxt(f'{result_path}/{filename.replace(".txt", f"_{method}.txt")}', anomaly_score)
        ...
        if benchmark == 'UCR_Anomaly_FullData':
        ...
        # ax.plot(reservoir.X, linewidth=0.2)
    ...
    time_path = f'../benchmarks/{benchmark}/analysis/runtime_{method}.csv'
