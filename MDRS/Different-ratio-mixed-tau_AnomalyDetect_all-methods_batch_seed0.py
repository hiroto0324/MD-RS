...
if __name__ == "__main__":
    ...
    # tau_list = np.linspace(0.01, 0.0225, 10)
    # print('tau list:', tau_list)
    mix_ratio_list = np.linspace(0, 1.0, 21)
    # mix_ratio_list = np.linspace(0.9, 1.0, 5)
    print(mix_ratio_list)
    # for iteration_idx in range(6):
    global seed
    # for iteration_idx, tau in enumerate(tau_list):
    for iteration_idx, mix_ratio in enumerate(mix_ratio_list):
        # if iteration_idx < 6:
        # if iteration_idx >= 6:
        #     continue
        ...
        ## ヘテロ時定数のシャッフル
        # param['tau'] = np.concatenate((np.full(int(param['Nx'] * (1 - mix_ratio)), 0.01),
        #                                np.full(int(param['Nx'] * mix_ratio), 0.0225)))  # 二つの値を混在させる→一番いい？
        param['tau'] = np.zeros(param['Nx'])
        param['tau'][:int(param['Nx'] * (1 - mix_ratio))] = 0.01
        # param['tau'][int(param['Nx'] * (1 - mix_ratio)):] = 0.0225 # ベスト？
        param['tau'][int(param['Nx'] * (1 - mix_ratio)):] = 0.025
        # param['tau'][int(param['Nx'] * (1 - mix_ratio)):] = 0.1
        ...
        result_path = f'../UCR_Anomaly_FullData/results/Reservoir/mixed-tau'
        plot_path = f'../UCR_Anomaly_FullData/plots/Reservoir/mixed-tau'
        ...
        for filename in os.listdir(folder_path):
            ...
            if match:
                ...
                np.savetxt(os.path.join(result_path, filename.replace(".txt", f"_{method}_mixed-tau{iteration_idx}.txt")), anomaly_score)
                ...
                plt.savefig(os.path.join(plot_path, filename.replace(".txt", f"_{method}_mixed-tau{iteration_idx}.png")))
                plt.close()

        time_path = f'../UCR_Anomaly_FullData/analysis/runtime_{method}_mixed-tau{iteration_idx}.csv'
